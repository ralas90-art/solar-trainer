...
# Helper function to get user_id from membership
async def get_active_membership(membership: Membership = Depends(get_current_membership)) -> Membership:
    return membership
...
async def get_kpi_definitions(
    ...
    """Get all KPI definitions for current user's company"""
    statement = select(KPIDefinition).where(
        KPIDefinition.user_id == membership.user_id,
        KPIDefinition.company_id == membership.company_id,
...
async def get_kpi_entries(
    ...
    statement = select(KPIEntry, KPIDefinition).join(
        ...
        KPIEntry.company_id == membership.company_id,
        KPIEntry.user_id == membership.user_id,
