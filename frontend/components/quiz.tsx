...
export function QuizModule({ quiz, onComplete }: QuizProps) {
    ...
    const correctAnswerIndex = (currentQuestion as any).correctAnswerIndex ?? (currentQuestion as any).correctIndex ?? 0
            ...
            <div className="glass-card p-8 rounded-2xl relative overflow-hidden">
                ...
                <div className="space-y-4">
                    {currentQuestion.options.map((option, idx) => {
                        ...
                        if (isAnswered) {
                            ...
                            } else if (idx === selectedOption && idx !== correctAnswerIndex) {
                        ...
                        return (
                            <button
                                ...
                                disabled={isAnswered}
                            className={`w-full min-h-[56px] text-left p-4 rounded-xl transition-all duration-200 flex items-center justify-between group ${stateStyles}`}
                            >
                                <span className="flex-1">{option}</span>
                                {isAnswered && idx === correctAnswerIndex && <CheckCircle className="w-5 h-5 text-green-400" />}
                                {isAnswered && idx === selectedOption && idx !== correctAnswerIndex && <XCircle className="w-5 h-5 text-red-400" />}
                            </button>
                        )
                    })}
                </div>
                ...
                <AnimatePresence>
                    {isAnswered && (
                        ...
                        >
                            <div className={`p-4 rounded-xl ${selectedOption === correctAnswerIndex ? 'bg-green-900/20 border border-green-900/30' : 'bg-blue-900/20 border border-blue-900/30'}`}>
                                <p className="text-sm font-bold mb-1 opacity-70 uppercase tracking-widest">{selectedOption === correctAnswerIndex ? 'Correct Analysis' : 'Correction'}</p>
                                <p className="text-slate-200 leading-relaxed">{currentQuestion.explanation || "No further explanation provided."}</p>
                            </div>
