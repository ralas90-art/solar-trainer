...
export function LeaderboardRow({
  ...
  return (
    ...
    >
      ...
      <td className="px-3 py-3 sm:px-4">
        <p className="text-[#94A3B8]">{rep.company || WHITE_LABEL.companyName}</p>
        <p className="font-hud text-[10px] uppercase tracking-[0.16em] text-[#64748B]">{rep.role || "REP"}</p>
      </td>
      ...
      <td className="px-3 py-3 text-right sm:px-4">
        <span className="inline-flex items-center gap-1 text-[#FFB300]">
          ...
          <span className="font-hud">{rep.currentStreak || 0}d</span>
        </span>
      </td>
      <td className="px-3 py-3 text-right sm:px-4">
        <span className="badge-amber">{rep.badgesEarned || 0}</span>
      </td>
    </tr>
  )
}
...
export function LeaderboardTable({
  ...
  entries: Array<{ rep: any; score: number; simulations: number }>
  title: string
  subtitle: string
  scoreLabel: string
          ...
          <tbody>
            {entries.map((entry, index) => (
              <LeaderboardRow
                ...
                onClick={onRowClick ? () => onRowClick(entry.rep.id) : undefined}
              />
            ))}
          </tbody>
        </table>
  ...
  return (
    <WidgetCard>
      ...
      <div className="mt-4 h-2 overflow-hidden rounded-full bg-white/5">
        <div className="brand-progress h-full" style={{ width: `${ratio}%` }} />
      </div>
    </WidgetCard>
  )
}
...
export function WeeklyImprovementCard({
  ...
  return (
    <WidgetCard>
      ...
      <div className="mt-4 h-2 overflow-hidden rounded-full bg-white/5">
        <div className="brand-progress h-full" style={{ width: `${Math.min(100, Math.max(0, weeklyProgress))}%` }} />
      </div>
    </WidgetCard>
  )
}
