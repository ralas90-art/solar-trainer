...
import { DynamicIcon } from "./icon-registry"
...
export function NotificationPill({
  ...
  return (
    ...
    >
      {iconKey ? <DynamicIcon iconKey={iconKey} className="h-3.5 w-3.5" /> : Icon ? <Icon className="h-3.5 w-3.5" /> : null}
      {label}
    </span>
  )
}
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div
          className={cn(
            ...
            accent === "lime"
              ? "border-[#FFB300]/20 bg-[#FFB300]/10 text-[#FFB300] secondary-glow"
              : "border-[#FF5722]/20 bg-[#FF5722]/10 text-[#FF5722] brand-glow"
          )}
        >
          {iconKey ? <DynamicIcon iconKey={iconKey} className="h-5 w-5" /> : Icon ? <Icon className="h-5 w-5" /> : null}
        </div>
      </div>
    </WidgetCard>
  )
  ...
  const content = (
    <>
      ...
      <div className="mt-5 h-2 overflow-hidden rounded-full bg-white/5">
        <div className="brand-progress h-full" style={{ width: `${progress}%` }} />
      </div>
    </>
  )
...
export function ScoreDisplayCard({
  ...
  return (
    <WidgetCard className="bg-[linear-gradient(180deg,rgba(255,87,34,0.08),rgba(18,18,18,0.9))]">
      <SectionEyebrow label={label} action={t("ai_scored", { en: "AI scored", es: "Calificado por IA" })} />
  ...
  const content = (
    <>
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="font-hud text-xs text-[#94A3B8]">
          {progress}% {t("complete", { en: "complete", es: "completado" })}
        </span>
      </div>
    ...
    <div className="flex items-center justify-between gap-4 rounded-2xl border border-white/5 bg-white/5 px-4 py-3">
      ...
      <div className="text-right">
        ...
        <p className="text-xs text-[#94A3B8]">
          {t("weekly_xp", { en: "weekly XP", es: "XP semanal" })}
        </p>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="h-2 overflow-hidden rounded-full bg-white/5">
        <div className="brand-progress h-full" style={{ width: `${value}%` }} />
      </div>
    </div>
  )
}
