            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {segments.map((segment, index) => (
                        <div key={index} className="bg-gradient-to-br from-slate-900 to-slate-950 border border-white/10 p-10 rounded-3xl flex flex-col">
                            <div className="text-[#FF5722] font-bold uppercase tracking-wider text-xs mb-4">{segment.role}</div>
                            <h3 className="text-2xl font-bold text-white mb-8">{segment.headline}</h3>
                            <ul className="space-y-4 mb-10 grow">
                                {segment.benefits.map((benefit, i) => (
                                    <li key={i} className="flex items-start gap-3">
                                        <CheckCircle2 className="w-5 h-5 text-[#FFB300] shrink-0 mt-0.5" />
                                        <span className="text-slate-300">{benefit}</span>
                                    </li>
                                ))}
                            </ul>
                            <Link href="/signup">
                                <Button className="w-full btn-primary h-12 rounded-xl">Get Started with {segment.role}</Button>
                            </Link>
                        </div>
                    ))}
                </div>
