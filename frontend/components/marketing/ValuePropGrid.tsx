...
export function ValuePropGrid() {
    ...
    return (
        <section id="product" className="py-24 bg-[rgba(18,18,18,0.68)]">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="text-center max-w-3xl mx-auto mb-16">
                    <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
                        Why Top Teams Choose {WHITE_LABEL.companyName}
                    </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {features.map((feature, index) => (
                        <div key={index} className="glass-circuit border border-white/5 p-8 rounded-2xl hover:border-[#FF5722]/30 transition-all duration-300 group">
                            <div className="w-12 h-12 bg-[#FF5722]/10 rounded-lg flex items-center justify-center mb-6 group-hover:bg-[#FF5722]/20 transition-colors">
                                <feature.icon className="w-6 h-6 text-[#FF5722] group-hover:text-[#FFD54F]" />
                            </div>
