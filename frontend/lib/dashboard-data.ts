...
const STORAGE_PREFIX_MODULE = `${WHITE_LABEL.companyName.toLowerCase().replace(/\s+/g, '_')}_training_progress_`
const STORAGE_PREFIX_AUDIO = `${WHITE_LABEL.companyName.toLowerCase().replace(/\s+/g, '_')}_audio_lesson_`
...
export function getDashboardStats(): DashboardStats {
  ...
  return {
    streak,
    level: 3, // Mock level for local storage fallback
    levelProgress: 45,
    totalXP: 1200,
    skillScore,
    simWinRate,
