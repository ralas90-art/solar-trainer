...
import { calculateLevel, getLevelProgress } from "./gamification-core"
import { WHITE_LABEL } from "./white-label.config"
import { getDemoReps, getDemoRepById } from "./demo-data"
...
function buildFallbackSnapshot(): AnalyticsSnapshot {
  ...
  return {
    ...
    totalTrainingDays: 18,
    certificationsEarned: 2,
    xpEarned: 4180,
    level: calculateLevel(4180),
    levelProgress: getLevelProgress(4180),
    leaderboardRank: 4,
    skills: {
...
export async function fetchAnalyticsSnapshot(
  ...
  if (WHITE_LABEL.isDemoMode) {
    // Intercept for demo reps
    const demoRep = getDemoRepById(userId);
    if (demoRep) {
        // Map demoRep to AnalyticsSnapshot
        return {
            ...buildFallbackSnapshot(), // Use fallback as base
            overallPerformanceScore: demoRep.performance.average_score,
            simulationsCompleted: demoRep.simulation_history.length,
            averageSimulationScore: demoRep.performance.average_score,
            currentTrainingStreak: demoRep.performance.current_streak,
            xpEarned: demoRep.performance.total_score,
            level: demoRep.performance.level,
            coachingInsights: demoRep.coaching_signals,
            simulationHistory: demoRep.simulation_history.map(s => ({
                id: s.simulation_id,
                scenarioName: s.scenario_name,
                score: s.score,
                skillsTested: ["discovery", "objections"], // Mock skills
  ...
  return {
    ...
    totalTrainingDays: stats.total_training_days || 18, // Fallback for demo
    certificationsEarned,
    xpEarned: stats.total_score,
    level: calculateLevel(stats.total_score),
    levelProgress: getLevelProgress(stats.total_score),
    leaderboardRank,
    skills,
