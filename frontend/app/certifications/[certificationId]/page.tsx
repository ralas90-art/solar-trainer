...
function getActiveUserId() {
  ...
  const prefix = WHITE_LABEL.companyName.toLowerCase().replace(/\s+/g, '_')
  const byKey = window.localStorage.getItem(`${prefix}_user_id`)
  if (byKey) return byKey
  const storedUser = window.localStorage.getItem(`${prefix}_user`)
  if (storedUser) {
    try {
      const parsed = JSON.parse(storedUser) as { id?: string }
      return parsed.id ?? "trainee"
    } catch {
      return "trainee"
    }
  }
