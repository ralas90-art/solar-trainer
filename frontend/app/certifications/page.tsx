...
import { Award, CheckCircle2, Clock3, ShieldCheck, Flame, Zap } from "lucide-react"
import { fetchAnalyticsSnapshot, AnalyticsSnapshot } from "@/lib/analytics-api"
import { getLevelInfo } from "@/lib/gamification-core"
...
export default function CertificationsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setSyncError(null)
      try {
        const [liveSnapshot, liveAnalytics] = await Promise.all([
          fetchCertificationSnapshot(userId),
          fetchAnalyticsSnapshot(userId)
        ])
        if (mounted) {
  ...
  const userXp = analytics?.xpEarned ?? 0
  const userLevelInfo = getLevelInfo(userXp)
  const userStreak = analytics?.currentTrainingStreak ?? 0
    ...
    >
      <div className="space-y-6">
        ...
        <section className="grid gap-6 xl:grid-cols-[1.7fr_1fr]">
          <div className="space-y-6">
            <WidgetCard>
              <SectionEyebrow label="Certification Tracks" action={`Core ${WHITE_LABEL.companyName} credential paths`} />
          ...
          <div className="space-y-6">
            ...
            />
            <CertificationGamification
              certificationXp={userXp}
              streakDays={userStreak}
              levelProgress={userLevelInfo.progress}
              milestoneLabel={ready.length > 0 ? "Ready for final assessment" : `Level ${userLevelInfo.level} Master`}
            />
          </div>
        </section>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            {earned.map((track) => (
              <CertificatePreviewCard
                key={track.id}
                track={track}
                downloadHref={`${apiUrl}/certificate/${encodeURIComponent(user?.id || "trainee")}`}
              />
            ))}
          </div>
        </section>
