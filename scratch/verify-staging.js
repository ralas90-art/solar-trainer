...
async function run() {
  ...
  try {
    ...
    for (const urlPath of pathsToVerify) {
      ...
      if (urlPath === '/team-hub') {
        const hasDemoReps = await page.evaluate(() => {
          return document.body.innerText.includes('Sarah Connor') || document.body.innerText.includes('John Doe');
        });
        console.log(`  - Contains Demo reps (Sarah/John): ${hasDemoReps}`);
        if (hasDemoReps) throw new Error('Data Leak! Demo representatives visible on /team-hub when demo mode is OFF.');
    ...
    const toggleButton = page.locator('div:has-text("Safe Demo Mode") button');
    ...
    const hasKPICounts = analyticsText.includes('342') || analyticsText.includes('Conversion');
    ...
    const debriefCount = await page.evaluate(() => {
      const reports = ['Sarah Connor', 'John Doe', 'Maria Rodriguez', 'Marcus Vance', 'Chloe Bennett'];
    ...
    const hasCerts = certText.includes('Elite Closer') || certText.includes('Technical Guru');
    console.log(`  - Certifications loaded: ${hasCerts}`);
    if (!hasCerts) {
      throw new Error('Demo certifications not found on /certifications page.');
    }
    ...
    await page.reload();
    await page.waitForTimeout(1000);
    const spanishBannerText = await page.locator('div:has-text("Modo de Demostración Activo")').first().innerText();
    console.log(`Spanish Banner text: "${spanishBannerText.replace(/\n/g, ' ')}"`);
    if (!spanishBannerText.includes('Modo de Demostración Activo')) {
    ...
    const repCountEnd = await page.evaluate(() => {
      const reps = ['Sarah Connor', 'John Doe'];
      return reps.filter(name => document.body.innerText.includes(name)).length;
    });
    console.log(`Demo reps present after disable: ${repCountEnd}`);
    if (repCountEnd > 0) {
      throw new Error('Demo reps are still present on /team-hub after disabling demo mode.');
    }
