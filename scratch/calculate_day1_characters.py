...
for mid in day1_mods:
    ...
    for f in expected_files:
        ...
        if stem in mapped_segments:
            ...
            print(f"WARNING: File {f} not matched in python segments!")
            
    results.append({
        'moduleId': mid,
        'moduleTitle': lesson_title,
        'charCount': module_char_count,
        'files': module_files
    })

print(f"\nTotal characters for Batch 1: {total_char_count:,}")

# Save detailed report to a JSON artifact so it's fully persistent and structured!
report_data = {
...
print("Report saved to artifacts/batch_1_dry_run_report.json")
